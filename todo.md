...
## MS11 - CRO 反作弊 + ROI 裂变 + 流失挽回弹药库

- [x] Epic 1: 防假自助反作弊引擎（IP/设备指纹 + 熔断 + 惩罚 0.5×）
- [x] Epic 1: 销售员 IP 登录记录 + IP 爆发阈值检测（同 IP 超 3 客户/天）
- [x] Epic 2: ROI 里程碑裂变奖励（累计 ¥500 触发 +50 元返佣）
...
## 三模块修复（客户管理/代客下单/库存管理）

- [ ] 修复1：客户管理新建表单必填校验（客户姓名+电话号码）
- [ ] 修复2：代客下单客户列表状态同步刷新（新建客户后无需刷新页面即可选择）
- [ ] 修复3a：库存管理新增SKU入口（仅管理员可见）
- [ ] 修复3b：管理员可编辑总产能和锁定产能字段
- [ ] TypeScript 0 错误验证
- [ ] 推送 GitHub main 分支

## MS12 Bug 修复（3 个已知问题）

- [x] 客户管理模块：新建客户表单必填校验（客户姓名 + 电话号码）
- [x] 代客下单模块：新建客户后客户选择列表刷新同步（local_customers 表 + getCustomers 合并查询 + invalidate）
- [x] 库存管理模块：新建 SKU 功能（管理员专属按钮 + createSku 后端 API）
- [x] 库存管理模块：总产能和锁定产能/配额编辑权限（管理员专属 updateCapacity API + 产能编辑 Modal）
- [x] 编写 vitest 测试覆盖 3 个修复点（server/bugfix-ms12.test.ts，13 tests passed）

## MS13 费用报销审批功能修复

- [x] 管理员审批按钮：在报销列表中为管理员显示审批通过/退回按钮
- [x] 退回原因：退回时必须填写退回原因（至少 5 字），保存到数据库
- [x] 销售员查看退回原因：销售员能看到退回原因说明（红色提示框）
- [x] 重新提交：销售员修改金额/说明后可重新提交（resubmit API）
- [x] 编写 vitest 测试覆盖审批流程（12 个测试全部通过）

## MS14 客户管理模块重构 + 费用报销模块修复

### 一、客户管理模块重构
- [x] 新增独立客户管理 CRUD 页面（/customers）
- [x] 客户类型选项（企业/个人/渠道/餐饮/工厂/批发/其他）
- [x] 根据客户类型自动生成客户编号（ENT-0001, CH-0001 等）
- [x] 字段级权限隔离：销售无权设置财务数据
- [x] 仅超级管理员/财务可设置信用额度和折扣率
- [x] 数据库 Schema 新增 customers 表（customerType, customerCode, creditLimit, discountRate 等）
- [x] 后端 CRUD API（list, create, get, update, updateFinance, deactivate）
- [x] 在 DashboardLayout 侧边栏添加客户管理入口
- [x] 路由注册（/customers）

### 二、费用报销模块修复
- [x] 新建报销表单：客户名称设为必填项（前端+后端校验）
- [x] 重构报销页面为“列表 + 新建”经典结构（默认显示历史记录）
- [x] 申请人可看到自己的所有历史报销记录
- [x] 数据可见性：普通员工只看自己的，管理员看所有
- [x] 已退回状态展示退回原因（红色提示框）
- [x] 已退回状态允许重新编辑提交
- [x] 编写 vitest 测试（12 个测试全部通过）

## MS15 修复客户管理页面侧边栏不显示

- [x] 检查 DashboardLayout 侧边栏菜单配置是否包含客户管理入口
